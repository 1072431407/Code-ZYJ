...
constexpr int kMaxChannelValue = 262143;
...
static void yuv420_to_rgba_fast_asm(const unsigned char* yuv420sp, int w, int h, unsigned char* rgb) {
    const unsigned char* yptr = yuv420sp;
    const unsigned char* uptr = yptr + w * h / 2 - 1;
    const unsigned char* vptr = uptr + w * h / 2 - 1;

    for (int i=0; i<h; i++) {
        int pUV = (i>>1) * w;
        for (int j=0; j<w; j++) {
            auto py = yptr + i*w+j;
            auto pu = uptr + pUV + (i>>1) * 2;
            auto pv = vptr + pUV + (i>>1) * 2;
            auto rgba_ptr = rgb + i*w+j;
            auto rgb_int = yuv2rgb(0xff & (*py), 0xff & (*pu), 0xff & (*pv));
            rgba_ptr[0] = (rgb_int >> 24) & 0xff;
            rgba_ptr[1] = (rgb_int >> 16) & 0xff;
            rgba_ptr[2] = (rgb_int >> 8) & 0xff;
            rgba_ptr[3] = rgb_int & 0xff;
        }
    }
}
