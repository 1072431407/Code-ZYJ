...
TreeNode::TreeNode(int type, TreeNode* Parent = 0){
	_data = type;
	_parent = Parent;
}
...
Tree::Tree(const int type, const vector<Tree*>& lit){
	...
	for (it = lit.begin(); it != lit.end(); it++){
		if (!((*it)->_nodes.empty())){//如果当前节点元素不为空
			...
			vector<TreeNode*>::iterator lit1 = tp->_nodes.begin();
			vector<TreeNode*>::iterator lit2 = tp->_nodes.end();
			vector<TreeNode*>::iterator lit3 = _nodes.end();
...
Tree::~Tree(){
	for (vector<TreeNode*>::iterator it = _nodes.begin(); it != _nodes.end(); it++){
		delete* it;
	}
}
...
bool Tree::operator ==(const Tree& t){
	...
	vector<TreeNode*>::iterator it = _nodes.begin();
	vector<TreeNode*>::const_iterator _it = t._nodes.begin();
...
int Tree::height(TreeNode* node){
	if (!node){
		...
		vector<TreeNode*> plist = node->_children;
		if (plist.size() == 0){
			return 0;
		}
		int hA = 0;
		for (vector<TreeNode*>::iterator it = plist.begin(); it != plist.end(); it++){
			int hB = height(*it);
			if (hB>hA){
				hA = hB;
			}
...
Iterator::Iterator(Tree* t, TreeNode* n){
	_tree = t;
	vector<TreeNode*>& nodes = _tree->_nodes;
	_lit = find(nodes.begin(), nodes.end(), n);//<algorithm> Members
}

Iterator::Iterator(Tree * t, vector<TreeNode*>::iterator lt){
	_tree = t;
	_lit = lt;
}
...
TreeNode* clone(TreeNode* node, List& nodes, TreeNode* nodep){
	...
	for (vector<TreeNode*>::iterator lt = l.begin(); lt != l.end(); lt++){
