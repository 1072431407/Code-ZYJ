...
typedef vector<TreeNode*> List;//重命名一个节点链表
...
public:
	...
	Tree(const int, const vector<Tree*>&);	//带参数构造函数
	~Tree();							//析构函数
...
private:
	vector<TreeNode*> _nodes;			//节点数组
	vector<TreeNode*>::iterator LIt;	//一个节点迭代器
	int height(TreeNode*);
...
private:
	...
	vector<TreeNode*>::iterator _lit;	//List Iterator
public:
	Iterator();								//默认构造函数
	Iterator(const Iterator&);				//复制构造函数
	Iterator(Tree*, TreeNode*);				//构造函数
	Iterator(Tree*, vector<TreeNode*>::iterator);//构造函数
	//运算符重载
	...
	typedef vector<TreeNode*>::iterator List;
	friend class Tree;
};
