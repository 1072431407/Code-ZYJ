...
def build_abi(input,output,abi,ndk,platform,toolchain,make_program,ext_params):
    ...
    # script = 'cmake -G"Unix Makefiles" -DANDROID_ABI=%s\
    #                                    -DANDROID_NDK=%s\
    #                                    -DANDROID_PLATFORM=%s\
    #                                    -DCMAKE_MAKE_PROGRAM=%s\
    #                                    -S %s\
    #                                    -B %s\
    #                                     %s'%(toolchain,abi,ndk,platform,make_program,src,output,ext_params_str)

    script = 'cmake -G"Unix Makefiles"\
                    -DCMAKE_ANDROID_ARCH_ABI=%s\
                    -DCMAKE_ANDROID_NDK=%s\
                    -DCMAKE_ANDROID_PLATFORM=%s\
                    -DCMAKE_MAKE_PROGRAM=%s\
                    -DCMAKE_TOOLCHAIN_FILE=%s\
                    -S %s\
                    -B %s\
                    %s'%(abi,ndk,platform,make_program,toolchain,input,output,ext_params_str)
    command(script)
